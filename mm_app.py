...
fig = go.Figure(go.Scattermapbox(
    ...
    #EDIT 2 LINES BELOW AND ENTER THE LONG AND LAT OF ROUTE 
    lon = [-78, -81],
    lat = [42, 34],
    #Add to the symbol in line below
    ##Will have to add symbols if there are more than 2 coordinates...
    marker = {'size': 10, 'symbol': ['airport', 'star']},
    #Add city names somehow
    ##Will also have to adjust if there are more than 2 ciites
    text = ["City 1", "City 2"],textposition = "bottom right"))
...
def get_route1(fro, to): #cities
    start_end_path=m.main(fro,to) #list of city names - start to end
    lons = []
    lats = []
...
#ORD ATL
    ...
    if (n_clicks1!=0):
        ...
        #app.update_layout(html.H6(children="Enter city number"))
        print(path)
        print(lats)
        print(lons)

        fig = go.Figure()
        fig.data=go.Scattergeo(
        lon=lons,
        lat=lats,
        mode='lines'
    )
        return u'From {} To {}'.format(input1, input2)
    if (n_clicks2!=0):
        #reset everything
        return "reset"
